...
kn_te = new_aBiofilm_connect
uk_te = new_aBiofilm_uncertain
val = kn_va + uk_va
train = kn_tr + uk_tr
test = kn_te + uk_te
random.shuffle(val)
...
with open('./datasets/drug_microbe/MDAD_aBiofilm/val.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in val:
        writer.writerow(row)
...
with open('./datasets/drug_microbe/MDAD_aBiofilm/train.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in train:
        writer.writerow(row)
...
with open('./datasets/drug_microbe/MDAD_aBiofilm/test.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in test:
        writer.writerow(row)
