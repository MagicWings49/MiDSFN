...
for set_ in ['train', 'val', 'test']:
    with open('./datasets/drug_microbe/MDAD+aBiofilm/' + set_ + '.csv', 'r', newline='\n', encoding='utf-8') as file:
    ...
    for i in range(1, len(dataset)):
        if dataset[i][2] == '0':
            ...
            dataset_dicard.append(dataset[i])
    with open('./datasets/drug_microbe/MDAD+aBiofilm_discard/' + set_ + '.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(st)
