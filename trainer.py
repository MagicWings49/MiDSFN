...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, shuffle_region=None,
                 opt_da=None, discriminator=None, experiment=None, alpha=1, **config):
        self.model = model
        self.optim = optim
        self.device = device
        self.epochs = config["SOLVER"]["MAX_EPOCH"]
        self.decoder = config["DECODER"]["NAME"]
        ...
        self.shuffle_region = shuffle_region if shuffle_region else None
        if opt_da:
        ...
        self.test_metrics = {}
        self.config = config
        self.output_dir = config["RESULT"]["OUTPUT_DIR"] + "/" + self.decoder
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            if auroc + auprc >= self.best_auc:
                self.best_model = copy.deepcopy(self.model)
                self.best_auroc = auroc
                self.best_auc = auroc + auprc
            ...
            test_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [auroc, auprc, f1, sensitivity, specificity,
                                                                            accuracy, thred_optim, test_loss]))
            self.test_table.add_row(test_lst)
            print('Test at Model of Epoch ' + str(self.current_epoch) + ' with test loss ' + str(test_loss),
                  " AUROC " + str(auroc) + " AUPRC " + str(auprc) + " Sensitivity " + str(
                      sensitivity) + " Specificity " + str(specificity) + " Accuracy " + str(
                      accuracy) + " Thred_optim " + str(thred_optim))
    ...
    def train_epoch(self):
        self.model.train()
        loss_epoch = 0
        num_batches = len(self.train_dataloader)-2 if self.shuffle_region else len(self.train_dataloader)
        if self.shuffle_region:
            for j in range(len(self.shuffle_region)):
                for i, (v_d, v_p, a_m, d_i, labels) in enumerate(tqdm(self.train_dataloader)):
                    if i >= self.shuffle_region[j][0] and i <= self.shuffle_region[j][1]:
                        self.step += 1
                        a_m = torch.stack(a_m, 0)

                        v_d, v_p, a_m, d_i, labels = v_d.to(self.device), v_p.to(self.device), a_m.to(
                            self.device), d_i, labels.float().to(self.device)
                        self.optim.zero_grad()
                        v_d, v_p, f, score = self.model(v_d, v_p, a_m, d_i)
                        if self.n_class == 1:
            ...
            for i, (v_d, v_p, a_m, d_i, labels) in enumerate(tqdm(self.train_dataloader)):
                ...
                v_d, v_p, a_m, d_i, labels = v_d.to(self.device), v_p.to(self.device), a_m.to(self.device), d_i, labels.float().to(self.device)
                self.optim.zero_grad()
                v_d, v_p, f, score = self.model(v_d, v_p, a_m, d_i)
                if self.n_class == 1:
                    n, loss = binary_cross_entropy(score, labels)
    ...
    def train_da_epoch(self):
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            batch_s[2] = torch.stack(batch_s[2], 0)
            batch_t[2] = torch.stack(batch_t[2], 0)
            v_d, v_p, a_m, d_i, labels = batch_s[0].to(self.device), batch_s[1].to(self.device), batch_s[2].to(
                self.device), batch_s[3], batch_s[4].float().to(self.device)
            v_d_t, v_p_t, a_m_t, d_i_t = batch_t[0].to(self.device), batch_t[1].to(self.device), batch_t[2].to(
                self.device), batch_t[3]
            self.optim.zero_grad()
            ...
            if self.current_epoch >= self.da_init_epoch:
                v_d_t, v_p_t, f_t, t_score = self.model(v_d_t, v_p_t, a_m_t, d_i_t)
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            self.model.eval()
            for i, (v_d, v_p, a_m, d_i, labels) in enumerate(data_loader):
                a_m = torch.stack(a_m, 0)
                v_d, v_p, a_m, d_i, labels = v_d.to(self.device), v_p.to(self.device), a_m.to(
                    self.device), d_i, labels.float().to(self.device)
                if dataloader == "val":
