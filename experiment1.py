...
#np.savetxt('./datasets/drug_microbe/MDAD/learning_part.txt', kn_tr + kn_va)
#np.savetxt('./datasets/drug_microbe/MDAD/testing_part.txt', kn_te)
...
with open('./datasets/drug_microbe/MDAD/val.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in val:
        writer.writerow(row)
...
with open('./datasets/drug_microbe/MDAD/train.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in train:
        writer.writerow(row)
...
with open('./datasets/drug_microbe/MDAD/test.csv', 'w', newline='',encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(st)
    for row in test:
        writer.writerow(row)
