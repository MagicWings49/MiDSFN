...
from utils import integer_label_microbe

class DTIDataset(data.Dataset):
    def __init__(self, list_IDs, df, max_drug_nodes=330):
    ...
    def __getitem__(self, index):
        ...
        #print(v_d.nodes())
        src, dst = v_d.edges()
        adjacency_matrix = torch.zeros((self.max_drug_nodes, self.max_drug_nodes), dtype=torch.float32)
        for i in range(v_d.num_edges()):
            adjacency_matrix[src[i], dst[i]] = 1.0
        v_p = self.df.iloc[index]['Microbe']
        name = v_p
        v_p = integer_label_microbe(v_p)
        y = self.df.iloc[index]["Y"]
        # y = torch.Tensor([y])
        return v_d, v_p, adjacency_matrix, name, y
