...
def main():
    ...
    shuffle_region = [[0, 216], [218, 1039], [1041, 1802]] if args.split == "aBiofilm_to_MDAD" else None

    if not cfg.DA.USE:
        trainer = Trainer(model, opt, device, training_generator, val_generator, test_generator, shuffle_region,
                          opt_da=None, discriminator=None, experiment=experiment, **cfg)
    else:
        trainer = Trainer(model, opt, device, multi_generator, val_generator, test_generator, shuffle_region,
                          opt_da=opt_da, discriminator=domain_dmm, experiment=experiment, **cfg)
    result = trainer.train()

    with open(os.path.join(cfg.RESULT.OUTPUT_DIR + "/" + cfg["DECODER"]["NAME"], "model_architecture.txt"), "w") as wf:
        wf.write(str(model))
